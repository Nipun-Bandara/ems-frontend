...
import { assignUser, UserDto } from "@/app/api/user-management/usersapi";
...
export default function AssignDialog({ user, open, onClose, onAssigned }: Props) {
  ...
  const canAssignDepartment = (currentUser?.roles ?? []).includes("SYSTEM_ADMIN");
  const canAssignRole = (currentUser?.roles ?? []).some((r: string) => r === "SYSTEM_ADMIN" || r === "DEPARTMENT_HEAD");
