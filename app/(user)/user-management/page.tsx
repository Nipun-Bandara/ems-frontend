...
const userManagementColumns: ColumnWithVisibility[] = [
	...
	{ field: "requestedRole", headerName: "Requested Role", flex: 1, minWidth: 160, show: true, visibleIn: ["unassigned"] },
	{ field: "joinedOn", headerName: "Joined On", flex: 1, minWidth: 140, show: true, visibleIn: ["unassigned"] },
	{
		...
		minWidth: 140,
		show: true,
		visibleIn: ["all", "unassigned"],
		renderCell: (params) => (
			<button
				className="rounded bg-primary px-3 py-1 text-sm text-white"
				onClick={() => params.row.onActionClick?.()}
			>
				Assign
			</button>
		),
		...
		<div className="flex h-full min-h-0 w-full bg-background">
			<div className="flex h-full min-h-0 w-full flex-col overflow-hidden p-5">
				...
				<div className="min-h-0 flex-1 overflow-hidden px-5 py-4 sm:px-6">
					<Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as TabKey)} className="h-full w-full flex-col gap-4">
						<TabsList className="w-fit bg-transparent p-0">
							...
							<TabsTrigger value="unassigned" className="gap-2 rounded-full px-4 py-2 text-sm font-medium">
								<UserCheck size={16} />
								Unassigned Users
							</TabsTrigger>
						</TabsList>
						...
						<TabsContent value="all" className="min-h-0 flex-1">
							<AllUsersTab columns={allUsersColumns} />
						</TabsContent>

						<TabsContent value="unassigned" className="min-h-0 flex-1">
							<UnassignedUsersTab columns={unassignedUsersColumns} />
						</TabsContent>
					</Tabs>
				</div>
			</div>
