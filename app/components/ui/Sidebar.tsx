        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className={cn(
                "fixed h-full w-full inset-0 bg-backgroundSecondary p-10 z-[100] flex flex-col justify-between",
                className
              )}
            >
              <div
                className="absolute right-3 top-3 z-50 text-textPrimary cursor-pointer"
                onClick={() => setOpen(!open)}
              >
                <X />
              </div>
  ...
  return (
    ...
    >
      ...
      <motion.span
        ...
        className="text-textPrimary text-sm transition duration-150 whitespace-pre inline-block !p-0 !m-0"
      >
        {link.label}
      </motion.span>
    </Link>
